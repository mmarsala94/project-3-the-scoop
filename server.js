...
function createComment(url, request){
  ...
  const response = {};
//May seem redundant at first. But, checking requestComment is an object does not mean it has all the properties filled out that we need!
  if(requestComment && requestComment.body && requestComment.username && database.users[requestComment.username] 
    ...
    //add comment that we just created to the database
    database.comments[comment.id] = comment;
    //add the comment id to the appropriate author and article
...
function upvoteComment(url, request){
  ...
  let savedComment = {};
  const response = {};
  if(!id || !username || !comment || !database.users[username]){
       response.status = 400;
  } else {
       savedComment = upvote(comment, username);
...
function downvoteComment(url, request){
  ...
  if(!id || !username || !comment || !database.users[username]){
       ...
       savedComment = downvote(comment, username);
       response.body = {comment: savedComment};
       response.status = 200;
  }
  return response;
...
function deleteComment(url, request){
  ...
  //remove from the user who commented and article commented on
  const userCommentIds = database.users[commentToDelete.username].commentIds;
  userCommentIds.splice(userCommentIds.indexOf(id), 1);
  const articleCommentIds = database.articles[commentToDelete.articleId].commentIds;
